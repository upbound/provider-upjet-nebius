...
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1beta1.AuthPublicKey_GroupVersionKind.String())
		}
	}, v1beta1.AuthPublicKey_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles AuthPublicKey managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta1.AuthPublicKey_GroupVersionKind.String())
	var initializers managed.InitializerChain
	for _, i := range o.Provider.Resources["nebius_iam_v1_auth_public_key"].InitializerFns {
		initializers = append(initializers, i(mgr.GetClient()))
	}
	rl := reconciliationpolicy.NewExponentialFailureRateLimiter(time.Second, 60*time.Second)
	eventHandler := handler.NewEventHandler(handler.WithDefaultRateLimiter(rl), handler.WithLogger(o.Logger.WithValues("gvk", v1beta1.AuthPublicKey_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1beta1.AuthPublicKey_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginFrameworkAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["nebius_iam_v1_auth_public_key"],
				...
				tjcontroller.WithTerraformPluginFrameworkAsyncMetricRecorder(metrics.NewMetricRecorder(v1beta1.AuthPublicKey_GroupVersionKind, mgr, o.PollInterval)),
	...
	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1beta1.AuthPublicKeyList{}, o.MetricOptions.PollStateMetricInterval,
		)
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(err, "cannot register MR state metrics recorder for kind v1beta1.AuthPublicKeyList")
		}
	}

	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta1.AuthPublicKey_GroupVersionKind), opts...)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1beta1.AuthPublicKey{}, eventHandler).
		Complete(
