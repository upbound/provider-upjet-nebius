...
func main() {
	var (
		app                   = kingpin.New("generator", "Run Upjet code generation pipelines for provider-upjet-nebius").DefaultEnvars()
	...
	pc, err := config.GetProvider(context.Background())
	kingpin.FatalIfError(err, "Cannot initialize the cluster-scoped provider configuration")
	pns, err := config.GetProviderNamespaced(context.Background())
