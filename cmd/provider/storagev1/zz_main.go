...
func main() {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-nebius-storagev1",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_storagev1(mgr, clusterOpts), "Cannot setup cluster-scoped Nebius controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_storagev1(mgr, namespacedOpts), "Cannot setup namespaced Nebius controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_storagev1(mgr, clusterOpts), "Cannot setup cluster-scoped Nebius controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_storagev1(mgr, namespacedOpts), "Cannot setup namespaced Nebius controllers")
	}
