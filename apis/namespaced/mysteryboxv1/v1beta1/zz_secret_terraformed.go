...
func (mg *Secret) GetTerraformResourceType() string {
	return "nebius_mysterybox_v1_secret"
}

// GetConnectionDetailsMapping for this Secret
func (tr *Secret) GetConnectionDetailsMapping() map[string]string {
	return map[string]string{"secret_version.payload[*].binary_value": "status.atProvider.secretVersion.payload[*].binaryValue", "secret_version.payload[*].key": "status.atProvider.secretVersion.payload[*].key", "secret_version.payload[*].string_value": "status.atProvider.secretVersion.payload[*].stringValue"}
}
