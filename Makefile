...
# NOTE(hasheddan): we force image building to happen prior to xpkg build so that
# we ensure image is present in daemon.
xpkg.build.provider-nebius: do.build.images
...
# object". Never set these in the provider's runtime environment.
generate $(TERRAFORM_PROVIDER_SCHEMA): export NEBIUS_TERRAFORM_PROVIDER_DISABLE_WRITE_ONLY = true
generate $(TERRAFORM_PROVIDER_SCHEMA): export NEBIUS_TERRAFORM_PROVIDER_DISABLE_DYNAMIC = true
